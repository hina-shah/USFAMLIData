...
proc corr data = &outdata.;
    var ga_edd;
    with p_sl_full;
/*p_gam_full p_hpglm_full p_rf_full */
...
%create_panels(indata=&outdata., suff=&fig_suf.);
...
%MACRO nn500_cn(
  ...
  PROC HPNEURAL DATA = &indata  ;
   ...
   TRAIN OUTMODEL = nnmod MAXITER=200;
  PROC HPNEURAL DATA = &indata;
   SCORE OUT=&outdata (DROP=_WARN_ RENAME=(p_&Y = p_nn500&SUFF)) MODEL = nnmod;
   ID _ALL_;
  RUN;
  PROC SQL; DROP TABLE nnmod; QUIT;
%MEND nn500_cn;
