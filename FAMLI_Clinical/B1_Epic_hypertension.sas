...
proc sql;
create table with_prev as
    select distinct a.filename, a.PatientID, a.studydate, a.DOC, a.delivery_date,
    ...
    on
        ...
        and (datepart(b.order_inst) <= a.DOC + 140)
;

%deleteRecordsOfPrevPregnancies(inputtable=with_prev,outputtable=medications_excl,datevariable=order_inst);

proc sql;
create table medications as
    select filename, PatientID, studydate, count(*) as med_count
    from medications_excl
    group by filename, PatientID, studydate;

delete * from medications where med_count < 1;

*Get ICD code counts;
proc sql;
create table with_prev as
    select distinct a.*, b.chr_htn, b.effective_date_dt
    from
        epic_maternal_info as a 
        inner join
...
delete * from diagnoses where count_occ < 1;

* Get the high blood pressure vitals;
create table with_prev as
    select distinct a.*, b.hypertension, datepart(b.recorded_time) as recorded_time format mmddyy10. 
    from
        epic_maternal_info as a 
        inner join
        bp_vitals as b
    on
        ...
        (datepart(b.recorded_time) <= a.DOC + 140)
;
...
create table vitals as
    select distinct filename, PatientID, studydate, sum(hypertension) as chr_htn_count_occ
    from vitals_excl
    group by PatientID, studydate, filename;
...
create table epic_maternal_info as
    select a.*, b.count_per_study >= 2 as chronic_htn 
    from
        epic_maternal_info as a 
        left join
...
create table labs as
    ...
    from
        ...
        (
            ...
            where
                (
                    prxmatch('/(protein\/creat ratio)/', lowcase(result_test_name)) > 0 and 
                    result_num>0.3
                )
        ) as b
        on
...
*Get medications;
proc sql;
create table medications_excl as
    ...
    from
        ...
        (
            ...
            where
                ...
                prxmatch('/(magnesium sulfate)/', lowcase(med_name)) > 0
                ) and 
                prxmatch('/inpatient/', lowcase(med_type))>0
        )as b
        on
            (a.PatientID = b.pat_mrn_id) and
...
create table per_study_counts_with_diag_labs as
    select a.*, b.count_occ 
    from
        per_study_counts as a 
        left join 
        diagnoses as b
    on
        a.PatientID = b.PatientID and
...
create table per_study_counts_with_diag_labs as
    select a.*, b.labs_count 
    from
        per_study_counts_with_diag_labs as a 
        left join 
        labs_count as b
    on
        a.PatientID = b.PatientID and
...
* Left join into the main table;
create table epic_maternal_info as
    select a.*,(b.count_per_study >= 2 and (b.labs_count>=1 or b.count_occ >=2)) as preg_induced_htn 
    from
        epic_maternal_info as a 
        left join
