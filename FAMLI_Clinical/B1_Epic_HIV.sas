...
(datepart(b.result_time) <= datepart(a.studydttm))
;

* Get counts for RNA Quant orders;
create table rna_quant_counts as
select filename, PatientID, studydttm, sum(hiv_orders) as hiv_quant_count_occ 
from
(
...
select filename, PatientID, studydttm, sum(hiv) as count_occ 
from
(
select a.*, b.hiv from
epic_maternal_info as a inner join
