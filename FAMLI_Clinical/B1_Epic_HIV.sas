...
create table labs_excl as
    ...
    from
        ...
        (
            ...
            where
            (
                prxmatch('/HIV/', lab_name) > 0 and 
                prxmatch('/^(positive|reactive|detected)/', lowcase(result)) eq 1
            ) or
            (
                prxmatch('/^(HIV RNA, QUANT)/', upcase(lab_name) ) > 0 and 
                not missing(result_num)
            )
        ) as b 
    on
...
create table labs as
    select distinct filename, PatientID, studydate, count(*) as lab_counts
    from labs_excl
...
create table rna_quant_counts as
    select distinct filename, 
        PatientID, 
        studydate, 
        sum(hiv_orders) as hiv_quant_count_occ
    from
    (
        select a.*, b.hiv_orders 
        from
            epic_maternal_info as a 
            inner join
            (
                select pat_mrn_id, result_time, 
                    prxmatch('/^(HIV RNA, QUANT)/', upcase(lab_name) ) > 0 as hiv_orders 
                from labs_pre 
            ) as b
        on
            (a.PatientID = b.pat_mrn_id) and
...
create table medications_excl as
    ...
    from
        ...
        (
            ...
            where
                prxmatch('/emtricitabine|dolutegravir|tenofovir|truvada|ritonavir|darunavir|raltegravir|norvir|prezista|lamivudine|zidovudine/', lowcase(med_name)) > 0
        ) as b
    on
        (a.PatientID = b.pat_mrn_id) and
...
create table epic_maternal_info as
    select a.*, b.count_per_study > 1 as hiv 
    from
        epic_maternal_info as a 
        left join
