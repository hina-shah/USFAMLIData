...
%deleteRecordsOfPrevPregnancies(inputtable=with_prev,outputtable=labs_excl,datevariable=result_time);

proc sql;
create table labs as
    select distinct filename, PatientID, studydate, count(*) as lab_count
    from labs_excl
...
create table medications as
    select distinct filename, PatientID, studydate, count(*) as med_count
...
create table with_prev as
    select distinct a.*, b.effective_date_dt, b.diabetes 
    from
        epic_maternal_info as a 
        inner join
...
create table diagnoses as
    select distinct filename, PatientID, studydate, DOC, delivery_date, sum(diabetes) as count_occ
    from diagnoses_excl
...
create table epic_maternal_info as
    select distinct a.*, b.count_per_study >= 1 as diabetes 
    from
        epic_maternal_info as a 
        left join
...
*Get labs;
proc sql;
create table labs_excl as
    select distinct a.filename, a.PatientID, a.studydate, b.lab_name, b.result 
    from
        epic_maternal_info as a 
        inner join
        (
...
create table medications_excl as
    select distinct a.filename, a.PatientID, a.studydate, b.med_Name 
    from
        epic_maternal_info as a 
        inner join
        (
