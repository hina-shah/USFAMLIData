...
proc sort data= famdat.&ga_final_table. out=WORK.SORTTempTableSorted(keep=PatientID episode_edd studydate ga_edd);
...
data famdat.b1_pregnancies_with_us;
...
proc contents data=famdat.b1_pregnancies_with_us varnum;
run;

*********** Statistics on the first ultrasound gestational ages ****************;
title 'Statistics on the first ultrasound gestational ages';
proc means data=FAMDAT.B1_PREGNANCIES_WITH_US n nmiss min mean median max std;
    var ga_1;
run;

proc univariate data=FAMDAT.B1_PREGNANCIES_WITH_US noprint;
    histogram ga_1;
run;
...
proc univariate data=famdat.&ga_final_table. noprint;
    histogram ga_edd;
run;
